...
public class Fortune {
    ...
    public static void main(String[] args ) {
        ...
        System.out.println("Введите число точек: ");
        Scanner sc = new Scanner(System.in);
        double start, stop;
        int N = sc.nextInt();
        ArrayList<Point> sites = new ArrayList<Point>(); // point list
        Random rnd = new Random();
        //add points on the display
        for (int i = 0; i < N; i++) {
            sites.add(new Point(rnd.nextDouble(), rnd.nextDouble()));
        }
        StdDraw.setCanvasSize(1350, 660);
        StdDraw.setScale(-0.001, 1.01);
        Stopwatch s = new Stopwatch();
        start = s.elapsedTime();
        Fortune v = new Fortune(sites, true);
        stop = s.elapsedTime();
        v.show();
    ...
    public Fortune(ArrayList<Point> sites, boolean animate) {
